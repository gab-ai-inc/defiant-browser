...
#define WIDEVINE_CDM_DOWNLOAD_URL_STRING \"https://redirector.gvt1.com/edgedl/widevine-cdm/${widevineConfig.configuredVersion}-linux-x64.zip\"
  ...
  signWinBinaries: () => {
    ...
    fs.copySync(path.join(config.outputDir, 'dissenter.exe'), path.join(dir, 'dissenter.exe'));
  ...
  generateWidevineSigFiles: () => {
    ...
    util.run('python', [sig_generator, '--input_file=' + path.join(src_dir, 'dissenter.exe'),
        '--flags=1',
        '--certificate=' + cert,
        '--private_key=' + key,
